...
public class JointsRotation : MonoBehaviour
{
    ...
    public void RotateJoints()
    {
        ...
        diffY = Mathf.Clamp(((float)rightFoot.position.y - (float)middlePoint.position.y) * -20, -60, 25);
        ...
        diffY = Mathf.Clamp(((float)leftFoot.position.y - (float)middlePoint.position.y) * -20, -60, 25);
